...
global ContextVec *performsearch()
  { sparams1 = getsparams(1);
    ...
    if (sparams1 -> smode == Any && sparams1 -> mask == 0)
      hfatals("Please enter either a search word or a grammatical category (or both).");
...
static ContextVec *matchall(SearchParams *sp)
  { ContextVec *cvec;
	...
	if (sp -> swords -> num == 0) hfatals("Bug: nothing to search for");
...
static ContextVec *matchone(int swd, Smode smo, int book, uint mask, uint morph)
  { ContextVec *cvec = new ContextVec;
	...
	char *s = getword(ref);
	unless (sscanf(s, "%d.%d.%d", &cx -> bk, &cx -> ch, &cx -> vn) == 3) hfatalfs("bug in matchall! (%s)", s);
	delete s;
      }
    return cvec;
  }
...
static void prsparams(SearchParams *sp)
  { if (sp == NULL) hfatals("Bug! null sp");
    printf("<table border>\n");
    unless (sp -> smode == Any)
