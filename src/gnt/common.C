...
#define ZIPFILE	    "/webdir/gnt/etc/gnt.bin.gz"
#define BINFILE	    "/webdir/gnt/cachedir/gnt.bin"
#define CACHEDIR    "/webdir/gnt/cachedir"
#define CACHEURL    "http://your.web.server/webdirforurl/gnt/cachedir"
...
global void openbinfile()
  { timenow = time(NULL);
    ...
    if (zipfiletime == 0) hfatalfs("%s does not exist!", ZIPFILE);
    unless (readable(BINFILE))
      { char cmd[MAXSTR+1];
	sprintf(cmd, "/usr/bin/gunzip -cq %s >%s", ZIPFILE, BINFILE);
	int code = system(cmd);
	if (code != 0) unlink(BINFILE);
      }
    binfile = fopen(BINFILE, "r");
    if (binfile == NULL) hfatalfs("can't open %s!", BINFILE);
    char magic[12];
...
global char *verseimage(Context ctxt)
  { char vid[9], fn[MAXSTR+1], img[MAXSTR+1];
    ...
    sprintf(fn, "%s/%s.gif", CACHEDIR, vid);
    unless (readable(fn)) makeversegif(ctxt, fn);
    touchfile(fn);
    sprintf(img, "<img src=%s/%s.gif ismap border=0>", CACHEURL, vid);
...
static void layoutverse(int vso, rect *rects)
  { int nwd = get1(vso);
	...
	if (currpt.y + CHHGT >= VBMHGT) hfatals("verse won't fit in bitmap!");
...
global char *grksimage(char *grks)
  { if (grks == NULL || grks[0] == '\0') return wordimage(0);	/* blank */
    ...
    sprintf(fn, "%s/%s.gif", CACHEDIR, uid);
    makewordgif(grks, fn);
    sprintf(img, "<img src=%s/%s.gif>", CACHEURL, uid);
    return copystring(img);
  }
...
global char *wordimage(int wdo)
  { char wid[16], fn[MAXSTR+1], img[MAXSTR+1];
    ...
    sprintf(fn, "%s/%s.gif", CACHEDIR, wid);
    unless (readable(fn))
    ...
    touchfile(fn);
    sprintf(img, "<img src=%s/%s.gif>", CACHEURL, wid);
    return copystring(img);
  }
...
global char *charimage(int ch)
  { char name[MAXSTR+1], fn[MAXSTR+1], img[MAXSTR+1];
    ...
    sprintf(fn, "%s/%s.gif", CACHEDIR, name);
    unless (readable(fn)) makechargif(ch, fn);
    touchfile(fn);
    sprintf(img, "<img src=%s/%s.gif>", CACHEURL, name);
    return copystring(img);
  }
...
static void writegif(gdImage *gd, char *gfn, int wid, int hgt)
  { int ssx = gd -> sx, ssy = gd -> sy;	    /* save width & height */
    ...
    FILE *fi = fopen(gfn, "w");
    if (fi == NULL) hfatalfs("can't create %s!", gfn);
    gdImagePng(gd, fi);
    fclose(fi);
...
static void setfiletime(char *fn, int t)
  { struct utimbuf utb;
    utb.actime = utb.modtime = (time_t) t;
    utime(fn, &utb);
  }
...
global char *bookname(int bk)
  { int bix = get3(16);
    int nbk = get1(bix);
    if (bk <= 0 || bk > nbk) hfatalfd("can't find %02d!", bk);
...
global int findvso(Context ctxt)
  { int cho = findcho(ctxt);	/* seek to chapter */
    int nvs = get1(cho);
    if (ctxt.vn <= 0 || ctxt.vn > nvs) hfatalfd("can't find %02d.%02d.%02d!", ctxt.bk, ctxt.ch, ctxt.vn);
...
global int findcho(Context ctxt)
  { int bix = get3(16);
    int nbk = get1(bix);
    if (ctxt.bk <= 0 || ctxt.bk > nbk) hfatalfd("can't find %02d!", ctxt.bk);
    int bko = get3(bix + 4 + 6*(ctxt.bk-1));
    int nch = get1(bko);
    if (ctxt.ch <= 0 || ctxt.ch > nch) hfatalfd("can't find %02d.%02d!", ctxt.bk, ctxt.ch);
    return get3(bko + 1 + 3*(ctxt.ch-1));
  }

global char *getword(int wdo)
  { int nch = get1(wdo);
    if (nch > MAXSTR) hfatals("Word too long!");
    char vec[MAXSTR+1];
