...
static void appends(char*, int&, char*, char* = NULL);
...
static void appends(char *vec, int &p, char *fmt, char *p1)
...
static void lockdb()
  { bool ok = lock_database(LOCKFILE);
    unless (ok) hfatals("Sorry, database is busy. Please try again.");
    atexit(unlockdb);
  }
  ...
  { Context ctxt = decodeid(getval("id"));  /* sets ctxt .bk .ch .vn .wn */
    if (ctxt.bk > 0 && ctxt.ch > 0)
	  ...
	  { char *coords = getval("xy"); point clickpt;
	    ...
	    if (ctxt.wn < 0) hfatals("You've clicked between two words. Press ``Back'' and try again.");
...
static void usage_error(char *msg)
  { // system("{ date; echo; env; echo; echo ==========; echo; } >> /webdir/gnt/Debug 2>/dev/null");
    ...
    printf("</ul>\n");
    printf("The Greek New Testament home page is <a href=%s>here</a>. <p>\n", GNTHOME);
    exit(0);
  }
...
static void dosearch()
  { ContextVec *cvec = performsearch();
	...
	writesearch();
	hfatals((char *)((occurnum == 0) ? "No occurrences." : "No more occurrences."));
      }
  }
...
static void printdetails(Context ctxt)
  { int vso = findvso(ctxt);	/* seek to verse */
    int nwd = get1(vso);
    if (ctxt.wn < 0 || ctxt.wn >= nwd) hfatalfd("can't find %02d.%02d.%02d.%02d!", ctxt.bk, ctxt.ch, ctxt.vn, ctxt.wn);
